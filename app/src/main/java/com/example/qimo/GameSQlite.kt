...
const val TABLE_NAME4 = "SignReward"
...
class GameSQlite(context:Context,version:Int): SQLiteOpenHelper(context, DB_NAME,null,version) {
    override fun onCreate(db: SQLiteDatabase?) {
        ...
        db?.execSQL("create table $TABLE_NAME3(_id integer primary key autoincrement, name text,Explanation text,image BLOB,Collection text,Date text)")
        db?.execSQL("create table $TABLE_NAME4(_id integer primary key autoincrement,Rewarddate text,Reward integer)")
